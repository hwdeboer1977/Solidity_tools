...
// We upgrade the contracts from MINTNFTERC20.SOL
// We need to use these upgrade libraries from OpenZeppelin
...
// HOW TO RUN THE CODE FOR ASSIGNMENT MODULE 2
...
contract MyERCTokenUpgrade is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    // We need to replace the constructor with initialize function 
    // initialSupply Initial token supply
    // authority Address of the authority contract (owner)
    function initialize(uint256 initialSupply, address authority) public initializer {
    ...
    // Function setOwner to set ownership to Authority later
    function setOwner(address authority) external onlyOwner {
        transferOwnership(authority);
    }
}

// STEP 2: CREATE AN ERC721 CONTRACT 
contract MyNFTUpgrade  is ERC721Upgradeable, OwnableUpgradeable {
    ...
    // Address of the authority contract (owner)
    function initialize(address authority) public initializer {
        __ERC721_init("NFT to buy with ERC20", "METANA");
        __Ownable_init();
        transferOwnership(authority); // Set authority as the owner
        currentTokenId = 0; // Starts minting from tokenId = 0
    }
...
contract AuthorityContractUpgrade  is OwnableUpgradeable {
 

    MyERCTokenUpgrade  public erc20Token;
    MyNFTUpgrade  public nftContract;

    // // Constructor below deploys both ERC20 and ERC721 contracts from above
    // constructor(uint256 initialSupply) {
    //     // Deploy the ERC20 token and NFT contract
    //     erc20Token = new MyERCTokenUpgrade (initialSupply, address(this));
    //     nftContract = new MyNFTUpgrade (address(this));

    // }

    // Rewrite the Authority contracy so that it does not create the MyERC20 and MyNFT contracts itself, 
    // modify the initialize function to accept the addresses of pre-deployed MyERCTokenUpgrade and MyNFTUpgrade contracts as arguments. 
    ...
    function initialize(address erc20Address, address nftAddress) public initializer {
        ...
        erc20Token = MyERCTokenUpgrade(erc20Address);
        nftContract = MyNFTUpgrade(nftAddress);
    }
